...
echo " "
echo "***************************************************************************"
echo "PostFreeSurferPipelineBatch   Optimizado neuroCOVID 2021 Pablo Billeke"
echo "***************************************************************************"
echo " "
...
echo "$@"
    ...
    QUEUE="" #  QUEUE="-q long.q"  #Just required on a grid engine encironment
#fi
...
for Subject in $Subjlist ; do
  ...
  if [ -n "${command_line_specified_run_local}" ] ; then
      ...
      queuing_command="${QUEUE}" #AQUI LO MODIFIQUE#"${FSLDIR}/bin/fsl_sub ${QUEUE}"
  fi
